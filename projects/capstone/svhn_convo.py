...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  # train_data, train_labels= load_mnist_data("training")
  # test_data, test_labels = load_mnist_data("testing")
  # valid_data, valid_labels = load_mnist_data("validation")
  ...
  #    tf.truncated_normal([IMAGE_SIZE // 4 * IMAGE_SIZE // 4 * 64, 512], stddev=0.04, seed=SEED))
  ...
  def model(data, train=False):
    ...
    fc_weights4 = tf.Variable(tf.truncated_normal(shape=[192, 10], stddev=1/192.0))
    fc_biases4 = tf.Variable(tf.constant(0.1, shape=[10]))
    
    softmax_linear = tf.add(tf.matmul(l4, fc_weights4), fc_biases4)
  ...
  with tf.Session() as sess:
    ...
    for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
      ...
      if step % EVAL_FREQUENCY == 0:
        ...
        print('Validation Accuracy: %.1f%%' % error_rate(eval_in_batches(valid_data, sess), valid_labels))
        sys.stdout.flush()
