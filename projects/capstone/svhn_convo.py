...
def train(train_data, train_labels, valid_data, valid_labels, test_data, test_labels, train_size, saved_weights_path):
  ...
  with tf.Session() as sess:
    ...
    for step in xrange(int(NUM_EPOCHS * train_size) // BATCH_SIZE):
      ...
      summary, _, l, lr, predictions = sess.run([merged, optimizer, loss, learning_rate, prediction], 
        feed_dict = feed_dict)
      
      if step % EVAL_FREQUENCY == 10:
        ...
        print('Minibatch Accuracy: %.1f%%' % error_rate(predictions, feed_dict[y_]))
      ...
      if step % EVAL_FREQUENCY == 99:
        ...
        summary, _, l, lr, valid_predictions = sess.run([merged, optimizer, loss, learning_rate, prediction], feed_dict=valid_feed_dict, options=run_options, run_metadata=run_metadata)
