...
def train_classification(train_data, train_labels, valid_data, valid_labels, test_data, test_labels, train_size, saved_weights_path):
  ...
  with tf.name_scope('input'):
    X = tf.placeholder(tf.float32, shape=[BATCH_SIZE, IMG_ROWS, IMG_COLS, NUM_CHANNELS])
  ...
  training = True
  logits, params = classification_head(X, BATCH_SIZE, training)
  ...
  with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
    ...
    for step in xrange(int(NUM_EPOCHS * train_size) // BATCH_SIZE):
      ...
      training = True 
      _, l, lr, predictions = sess.run([optimizer, loss, learning_rate, logits], feed_dict = feed_dict)
      ...
      if step %  1000 == 0:
        ...
        print('Validation Accuracy: %.2f%%' % error_rate(valid_predictions, valid_feed_dict[y_]))
      ...
      if step % 100 == 0:
        ...
        train_error_rate = error_rate(predictions, feed_dict[y_])
